<script>
	...
	const getButtonColor = (state) => {
		switch(state) {
			case 'Process Dataset': return '#060';
			case 'Cancel': return '#700';
		}
		return '#006';
	};
	...
	const drawView = {
		...
		toViewSpace: function (x, y) {
			return {
				...
				y: ((this.canvasSize.y - 1 -y * Math.pow(2, this.scale.y)) + this.offset.y)
			};
		},	
		// from view space
		toPixelSpace: function (x, y) {
			return {
				...
				y: (this.canvasSize.y + 1 -  (y - this.offset.y)) / Math.pow(2, this.scale.y)
			};
		}
	};
	...
	const generationState = {
		...
		queryNewLogs: function() {
			...
			req.send(JSON.stringify(
				{"beginMicros": this.logsLastRetrieved}
			));
		}
	};
	...
	const correctMod = (val, mod) => {
        ...
        if (Math.floor(val) < 0 && modVal) return mod - modVal;
          ...
          const cursorPX = drawView.toPixelSpace(drawView.cursor.x, drawView.cursor.y);
          ...
          x: floorOnInterval(cursorPX.x, config.SpatialConfig.InputTileSize[0]) / config.SpatialConfig.InputTileSize[0],
          y: floorOnInterval(cursorPX.y, config.SpatialConfig.InputTileSize[1]) / config.SpatialConfig.InputTileSize[1]
          };
          const hoveredOutputTile = {
          x: floorOnInterval(cursorPX.x, config.SpatialConfig.OutputTileSize[0]) / config.SpatialConfig.OutputTileSize[0],
          y: floorOnInterval(cursorPX.y, config.SpatialConfig.OutputTileSize[1]) / config.SpatialConfig.OutputTileSize[1]
          };

          // draw cursor
          {
          const textSS = drawView.toPixelSpace(drawView.cursor.x + 6, drawView.cursor.y - 10);
          const textSS1 = drawView.toPixelSpace(drawView.cursor.x + 6, drawView.cursor.y - 25);
          const textSS2 = drawView.toPixelSpace(drawView.cursor.x + 6, drawView.cursor.y - 40);
          ctx.strokeStyle = '#fff4';
          vertical(cursorPX.x);
          horizontal(cursorPX.y);
          text(textSS.x, textSS.y,	'Pixel      : ' + humanize(cursorPX.x) + ', ' + humanize(cursorPX.y));
          text(textSS1.x, textSS1.y,	'Input Tile : ' + humanize(hoveredInputTile.x) + ', ' + humanize(hoveredInputTile.y));
          text(textSS2.x, textSS2.y,	'Output Tile: ' + humanize(hoveredOutputTile.x) + ', ' + humanize(hoveredOutputTile.y));
          }
          ...
          {
          fillRect(
          Math.floor(cursorPX.x),
          Math.floor(cursorPX.y),
          1,
          1,
          '#ffff'
          );
          ...
          const beginX = floorOnInterval(config.SpatialConfig.OutputPixelRange.min[0], intervalX) / intervalX;
          const endX = ceilOnInterval(config.SpatialConfig.OutputPixelRange.max[0] - 1, intervalX) / intervalX;
          const beginY = floorOnInterval(config.SpatialConfig.OutputPixelRange.min[1], intervalY) / intervalY;
          const endY = ceilOnInterval(config.SpatialConfig.OutputPixelRange.max[1] - 1, intervalY) / intervalY;
        ...
        const intervalX = config.SpatialConfig.OutputTileSize[0] * Math.pow(2, viewConfig.previewOutputLevel);
        const intervalY = config.SpatialConfig.OutputTileSize[1] * Math.pow(2, viewConfig.previewOutputLevel);
        ...
        config.SpatialConfig.OutputPixelRange.max[0] - config.SpatialConfig.OutputPixelRange.min[0],
        config.SpatialConfig.OutputPixelRange.max[1] - config.SpatialConfig.OutputPixelRange.min[1],
        '#44f3'
        );
        }
        };
        ...
        changeScaleAround(drawView, ev.wheelDeltaY * 0.0005, ev.clientX, ev.clientY);
        drawConfig(config, getCanvas());
        //clickSound.play();
        });
...
<main>
	...
	<div class='leftBlock'>
		...
		<button class='executeButton' style='background-color:{getButtonColor(buttonText)}' on:click={buttonExecute}>
			{buttonText}
		</button>
